...
var getFullNames = function(customersArray) {
    var newArray = []
    for (var i = 0; i < customersArray.length; i++) {
        var insideArrays = customersArray[i] 
        newArray.push(insideArrays.first + " " + insideArrays.last) 
    }
    return newArray
}
...
var getCounts = function(strOfWords) {
    var arrOfWords = strOfWords.toLowerCase().split(" ")
    var wordsObject = {}
...
var GenerateDog = function (inputName) {
        ...
        this.speak = function(string) {
            ...
            for (var i=0; i<oldString.length; i++) {
                var word = (oldString[i])
                //word  = (word.charAt(0) = "r")
                dogString += "r" + word.substring(1) + " "
            }
            return dogString 
        }
}
...
var pluck = function(objectsArray,inputProperty) {
    var prop = []
    for (var i = 0; i <objectsArray.length; i++) {
        var stoogeObject = objectsArray[i]
        log(stoogeObject[inputProperty])
        prop.push(stoogeObject[inputProperty])
    }
    return prop
}
...
var reverseObjects = function(arrOfObjects) {
    var newArray = []
    var newObjects = {}
    for (var i=0; i < arrOfObjects.length; i++) {
        var individualObjects = arrOfObjects[i]
        for (var prop in individualObjects) {
        var value = prop
        var newProperty = individualObjects[prop]
        newObjects[newProperty] = value
...
var politeObject = {
    ...
    personalize: function (inputFunction) {
        return ("Hi, my name is Frank, and the result is " + (inputFunction("h")) + ".")
    }
}
